...
class ShapleyValues():
    def __init__(self, unlocked_instructions, locked_instructions, value_fun, value_kwargs_dict, shap_sample_frac, shap_sample_reps, shap_batch_size, evaluate_value_only_once, shap_sample_seed, memory, callback, delta_exponent, name, silent):
        # setup instructions
    ...
    def _eval_Si_total_list(self):
        ...
        if self._evaluate_value_only_once:
            Si_effective_total_list = list(list(x) for x in set(tuple(x) for x in self.Si_total_list_)) # remove duplicates
        else:
            Si_effective_total_list = self.Si_total_list_
        if self._shap_batch_size is not None:
            Si_effective_total_list = [Si_batch_list.tolist() for Si_batch_list in np.array_split(Si_effective_total_list, np.ceil(len(Si_effective_total_list)/self._shap_batch_size))]
        total = sum([len(Si_batch_list) for Si_batch_list in Si_effective_total_list]) if self._shap_batch_size is not None else len(Si_effective_total_list)
