...
def value_fun_batch_wrapper(qc_data, num_qubits, S_list, quantum_instance, wrapped_value_fun, **kwargs):
    ...
    return value_fun_batch_wrapper_base(S_list, wrapped_value_fun, qc_data=qc_data, num_qubits=num_qubits, quantum_instance=quantum_instance, **kwargs)
...
def value_Expr(qc_data, num_qubits, S, quantum_instance, rng, num_samples, bins, p_lim_fun):
    ...
    if len(param_def_dict) > 0:    
        ...
        for idx in range(num_samples):
            param_def_dict_list = [{p: rng.uniform(p_lim_fun(p)[0], p_lim_fun(p)[1]) for p in param_def_dict.keys()} for _ in range(2)]
            sv1, sv2 = evaluate_circuits([qc for _ in range(2)], param_def_dict_list, quantum_instance, counts=False, sv=True, add_measurement=False)
